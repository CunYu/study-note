本文介绍了狄克斯特拉算法相关知识。内容仅供参考使用，有不足之处请及时指出，也欢迎大家交流探讨。
...
public static void dijkstra(Graph graph, String startVertex, String endVertex) {
    ...
    while (!startVertex.equals(weightRecordMap.get(tempVertex).getFatherVertex())) {
        vertexStack.push(weightRecordMap.get(tempVertex).getFatherVertex());
        weightSum += weightRecordMap.get(tempVertex).getWeight();
        tempVertex = weightRecordMap.get(tempVertex).getFatherVertex();
    }

    System.out.println("起始顶点到结束顶点的最短路径为" + weightSum);
