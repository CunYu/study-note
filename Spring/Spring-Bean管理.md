...
6 判断Bean是否实现了BeanPostProcessor接口，如实现，则调用postProcessAfterInitialization方法。
...
该注解声明方法有参数时，Spring会自动去为该方法参数从Spring容器总寻找其所需要的依赖并满足其依赖的需求。
...
当声明在Bean创建时，会被Bean声明一个限定符，默认限定符与Bean ID相同，也可以注解中声明限定符。
